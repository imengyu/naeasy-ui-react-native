...
export class PortalHost extends React.Component<PortalHostProps> {
  ...
  _addTypeListenerSubscription: EmitterSubscription|null = null;
  _removeTypeListenerSubscription: EmitterSubscription|null = null;

  componentDidMount() {
    const manager = this._manager;
    const queue = this._queue;

    this._addTypeListenerSubscription = TopViewEventEmitter.addListener(addType, this._mount);
    this._removeTypeListenerSubscription = TopViewEventEmitter.addListener(removeType, this._unmount);
  ...
  componentWillUnmount() {
    if (this._addTypeListenerSubscription) {
      this._addTypeListenerSubscription.remove();
      this._addTypeListenerSubscription = null;
    }
    if (this._removeTypeListenerSubscription) {
      this._removeTypeListenerSubscription.remove();
      this._removeTypeListenerSubscription = null;
    }
  }
