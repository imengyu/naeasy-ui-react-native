...
import { ColumnView, CellGroup, Button, RowView, Text } from '../lib';
      ...
      <ScrollView>
        <ColumnView center style={TestStyles.PaddingS}>
          <CellGroup title="基础用法">
            <Text style={TestStyles.TitleText}>按钮支持 default、primary、success、warning、danger 五种类型，默认为 default。</Text>
            <RowView wrap style={TestStyles.PaddingS}>
              ...
              <Button text="warning" type="warning" onPress={()=>console.log('点击了！')} />
            </RowView>
          </CellGroup>
          <CellGroup title="朴素按钮">
            <Text style={TestStyles.TitleText}>通过 plain 属性将按钮设置为朴素按钮，朴素按钮的文字为按钮颜色，背景为白色。</Text>
            <RowView wrap style={TestStyles.PaddingS}>
              <Button plain text="primary" type="primary" onPress={()=>console.log('点击了！')} />
              ...
              <Button plain text="warning" type="warning" onPress={()=>console.log('点击了！')} />
            </RowView>
          </CellGroup>
          <CellGroup title="禁用状态">
            <Text style={TestStyles.TitleText}>通过 touchable=false 属性来禁用按钮，禁用状态下按钮不可点击。也可不提供 onPress 函数，此时按钮也无法响应点击。</Text>
            <RowView wrap style={TestStyles.PaddingS}>
          ...
          <CellGroup title="自定义颜色">
            <RowView style={TestStyles.PaddingS} wrap>
              <Button text="自定义颜色" type="custom" color="#f00" pressedColor="#ff0" textColor="#fff" onPress={()=>console.log('点击了！')} />
            </RowView>
          </CellGroup>
          <CellGroup title="加载状态">
            <RowView wrap style={TestStyles.PaddingS}>
              <Button loading={true} type="primary" icon="top-filling" onPress={()=>console.log('点击了！')} />
              <Button loading={true} loadingText="加载中" type="success" onPress={()=>console.log('点击了！')} style={{ marginLeft: 5 }} />
            </RowView>
          </CellGroup>
          <CellGroup title="按钮形状">
            <Text style={TestStyles.TitleText}>通过 square 设置方形按钮，通过 round 设置圆形按钮。</Text>
            <RowView wrap style={TestStyles.PaddingS}>
              <Button shape="square" text="方形按钮" type="primary" onPress={()=>console.log('点击了！')} />
          ...
          <CellGroup title="图标按钮">
            <Text style={TestStyles.TitleText}>通过 icon 属性设置按钮图标，支持 Icon 组件里的所有图标，也可以传入图标 URL。</Text>
            <RowView wrap style={TestStyles.PaddingS}>
              <Button icon="top-filling" type="primary" onPress={()=>console.log('点击了！')} />
          ...
          <CellGroup title="按扭尺寸">
            <Text style={TestStyles.TitleText}>支持 large、normal、small、mini 四种尺寸，默认为 normal。</Text>
            <RowView wrap align="center" style={TestStyles.PaddingS}>
